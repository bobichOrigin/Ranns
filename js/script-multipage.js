...
;
    ...
    const tabsBtns = document.querySelectorAll('[data-tab-btn-type]');

    tabsBtns.forEach(item => {
        item.addEventListener('click', e => {
            if (!item.classList.contains('_active')) {
                document.querySelector('.info-tabs__header-item._active').classList.remove('_active');
                item.classList.add('_active');
                let tabType = item.dataset.tabBtnType;
                document.querySelector('.info-tabs__item._active').classList.remove('_active');
                document.querySelector(`[data-tab-type="${tabType}"]`).classList.add('_active');
            }
        });
    });
