...
window.onload = function () {
    ...
    function documnetActions(e) {
        ...
        // Load more sites
        if (targetElement.classList.contains('sites-landing__footer-btn')) {
            getSites(targetElement);
            e.preventDefault();
        }
    }
...
;
    ...
    function createSitesSliders() {
        const sitesItems = document.querySelectorAll('.sites-landing__item-header');

        sitesItems.forEach(item => {
            if (!item.classList.contains('_created')) {
                let sitesSlider = item.querySelector('.sites-landing__item-slider');

                let itemSiteArrowPrev = item.querySelector('.sites-landing__item-arrow.sites-landing__item-arrow--prev');
                let itemSiteArrowNext = item.querySelector('.sites-landing__item-arrow.sites-landing__item-arrow--next');

                new Swiper(sitesSlider, {
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
                    speed: 800,
                    ...
                    // Arrowss
                    navigation: {
                        prevEl: itemSiteArrowPrev,
                        nextEl: itemSiteArrowNext,
                    },
                });

                item.classList.add('_created');
            }
        });
    }
    ...
    async function getSites(button) {
        if (!button.classList.contains('_hold')) {
            button.classList.add('_hold');
            const file = "json/sites.json";
            let response = await fetch(file, {
                method: "GET"
            });
            if (response.ok) {
    ...
    function loadSites(data) {
        const sitesBody = document.querySelector('.sites-landing__body');

        data.sites.forEach(item => {
            let sitesTemplateHeaderStart = `<div class="sites-landing__item-header">`;
            let sitesTemplateHeaderEnd = `</div>`;

            let sitesTemplateSliderStart = `<div class="sites-landing__item-slider _swiper">`;
            let sitesTemplateSliderEnd = `</div>`;

            let sitesImages = item.images;

            let sitesTemplateImages = ' ';

            if (sitesImages.length > 1) {
                sitesImages.forEach(item => {
                    let sitesTemplateImagesItem = `
                        <div class="sites-landing__item-img">
                            <img src="img/landing/sites/${item}" alt="">
                        </div>
                    `;

                    sitesTemplateImages += sitesTemplateImagesItem;
                });
            } else {
                let sitesTemplateImagesItem = `
                <div class="sites-landing__item-img">
                    <img src="img/landing/sites/${sitesImages[0]}" alt="">
                </div>
                `;
            ...
            let sitesTemplateSlider = ' ';
            sitesTemplateSlider += sitesTemplateSliderStart;
            sitesTemplateSlider += sitesTemplateImages;
            sitesTemplateSlider += sitesTemplateSliderEnd;

            let sitesTemplateArrows = `
                <div class="sites-landing__item-arrows">
                    <button class="sites-landing__item-arrow sites-landing__item-arrow--prev _icon-arrow" type="button"></button>
                    <button class="sites-landing__item-arrow sites-landing__item-arrow--next _icon-arrow" type="button"></button>
                </div>
            `;

            let sitesTemplateHeader = ' ';
            sitesTemplateHeader += sitesTemplateHeaderStart;
            sitesTemplateHeader += sitesTemplateSlider;
            sitesTemplateHeader += sitesTemplateArrows;
            sitesTemplateHeader += sitesTemplateHeaderEnd;

            let sitesTemplateBodyStart = `<div class="sites-landing__item-body">`;
            let sitesTemplateBodyEnd = `</div>`;

            let sitesTemplateRowStart = `<div class="sites-landing__item-row">`;
            let sitesTemplateRowEnd = `</div>`;

            let sitesTemplateTasksTitle = `<div class="sites-landing__item-row-title">Задачи:</div>`;
            let sitesTemplateResultsTitle = `<div class="sites-landing__item-row-title">Итоги:</div>`;
            let sitesTemplateLinkTitle = `<div class="sites-landing__item-row-title">Сайт:</div>`;

            let sitesTemplateTasksListStart = `<ol class="sites-landing__item-row-values sites-landing__item-row-values--list">`;
            let sitesTemplateTasksListEnd = `</ol>`;

            let sitesTemplateTasks = ' ';

            let sitesTasks = item.tasks;

            if (sitesTasks.length > 1) {
                sitesTasks.forEach(item => {
                    let sitesTemplateTasksItem = `
                    <li class="sites-landing__item-row-value">
                        ${item}
                    </li>
                    `;

                    sitesTemplateTasks += sitesTemplateTasksItem;
                });
            } else {
                let sitesTemplateTasksItem = `
                <li class="sites-landing__item-row-value">
            ...
            let sitesTemplateTasksList = ' ';
            sitesTemplateTasksList += sitesTemplateTasksListStart;
            sitesTemplateTasksList += sitesTemplateTasks;
            sitesTemplateTasksList += sitesTemplateTasksListEnd;

            let sitesTemplateTasksRow = ' ';
            sitesTemplateTasksRow += sitesTemplateRowStart;
            sitesTemplateTasksRow += sitesTemplateTasksTitle;
            sitesTemplateTasksRow += sitesTemplateTasksList;
            sitesTemplateTasksRow += sitesTemplateRowEnd;

            let sitesTemplateResultsListStart = `<div class="sites-landing__item-row-values">`;
            let sitesTemplateResultsListEnd = `</div>`;

            let sitesTemplateResults = ' ';

            let sitesResults = item.results;

            if (sitesResults.length > 1) {
                sitesResults.forEach(item => {
                    let sitesTemplateResultsItem = `
                    <div class="sites-landing__item-row-value">
                        ${item}
                    </div>
                    `;

                    sitesTemplateResults += sitesTemplateResultsItem;
                });
            } else {
                let sitesTemplateResultsItem = `
                <div class="sites-landing__item-row-value">
            ...
            let sitesTemplateResultsList = ' ';
            sitesTemplateResultsList += sitesTemplateResultsListStart;
            sitesTemplateResultsList += sitesTemplateResults;
            sitesTemplateResultsList += sitesTemplateResultsListEnd;

            let sitesTemplateResultsRow = ' ';
            sitesTemplateResultsRow += sitesTemplateRowStart;
            sitesTemplateResultsRow += sitesTemplateResultsTitle;
            sitesTemplateResultsRow += sitesTemplateResultsList;
            sitesTemplateResultsRow += sitesTemplateRowEnd;

            let sitesTemplateLinkListStart = `<div class="sites-landing__item-row-values">`;
            let sitesTemplateLinkListEnd = `</div>`;

            let sitesLink = item.link;

            let sitesTemplateLink = `
            <div class="sites-landing__item-row-values">
                <div class="sites-landing__item-row-value">
                    <a href="${sitesLink}">${sitesLink}</a>
                </div>
            </div>
            `;

            let sitesTemplateLinkRow = ' ';
            sitesTemplateLinkRow += sitesTemplateRowStart;
            sitesTemplateLinkRow += sitesTemplateLinkTitle;
            sitesTemplateLinkRow += sitesTemplateLink;
            sitesTemplateLinkRow += sitesTemplateRowEnd;

            let sitesTemplateBody = ' ';
            sitesTemplateBody += sitesTemplateBodyStart;
            sitesTemplateBody += sitesTemplateTasksRow;
            sitesTemplateBody += sitesTemplateResultsRow;
            sitesTemplateBody += sitesTemplateLinkRow;
            sitesTemplateBody += sitesTemplateBodyEnd;

            let sitesId = item.id;

            let sitesTemplateStart = ` <div data-sites-id="${sitesId}" class="sites-landing__item">`;
            let sitesTemplateEnd = `</div>`;

            let sitesTemplate = ' ';
            sitesTemplate += sitesTemplateStart;
            sitesTemplate += sitesTemplateHeader;
            sitesTemplate += sitesTemplateBody;
            sitesTemplate += sitesTemplateEnd;

            sitesBody.insertAdjacentHTML('beforeend', sitesTemplate);

            //BildSlider
            let sliders = sitesBody.querySelectorAll('._swiper');
            if (sliders) {
                for (let index = 0; index < sliders.length; index++) {
                    let slider = sliders[index];
                    if (!slider.classList.contains('swiper-bild')) {
    ...
    faqBtns.forEach(item => {
        item.addEventListener('click', e => {
            let faqBtnParent = item.closest('.faq__item');

            if (!item.classList.contains('_active')) {
                if (document.querySelector('.faq__item._active')) {
                    document.querySelector('.faq__item._active').classList.remove('_active');
                }
                faqBtnParent.classList.add('_active');
            } else if (item.classList.contains('_active')) {
                faqBtnParent.classList.remove('_active');
            }
        });
    });
